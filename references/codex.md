...
Before composing a command, run `codex exec --help`. Use <https://learn.chatgpt.com/docs/non-interactive-mode> and the [`codex exec` command reference](https://learn.chatgpt.com/docs/developer-commands?surface=cli#cli-codex-exec) to resolve behavior that the help output does not explain.
...
- Use `--ignore-user-config` or `--ignore-rules` only for controlled automation that must exclude those inputs. `--ignore-rules` excludes execpolicy `.rules` files, not project instructions.

## Permissions

- Set `approval_policy="never"` with `-c` for a headless run so an unavailable human prompt fails closed. Do not use an interactive approval policy in unattended automation.
...
- Check `codex exec --help` for a native turn or iteration limit. When none is available, wrap every run in the host's timeout mechanism, for example `timeout 600 codex exec ...`.
...
- Without `--json`, Codex writes progress to stderr and the final agent message to stdout. Use this mode only when a human or simple pipeline needs the final text.
- Use `--json` for programmatic callers. It emits JSONL events, not one JSON object; ignore unknown event fields.
- Capture `thread_id` from `thread.started`. Accept the run only after `turn.completed`; reject `turn.failed`, `error`, a malformed stream, or a stream that ends without a terminal event.
...
- Continue a recorded session with `codex exec resume <session_id> "<follow-up>"`, or use `codex exec resume --last "<follow-up>"` for the most recent session in the current working directory.
...
- Let the first run use existing Codex authentication. If it reports that authentication is required, stop and tell the user to run `codex login`; do not attempt to authenticate.
