...
- `--sandbox danger-full-access` only in an externally isolated runner. Do not use `--dangerously-bypass-approvals-and-sandbox` merely to resolve a denied command.
- Do not use the deprecated `--full-auto` compatibility flag; select the sandbox explicitly.

## Bounding

- Wrap every run in the host's timeout, such as `timeout 600 codex exec ...`.
...
- `--json` for programmatic callers. It emits JSONL events, not one JSON object; ignore unknown event fields.
...
- Let the first run use existing Codex authentication. If authentication is required, stop and tell the user to run `codex login`; do not authenticate on their behalf.
...
- Consult `codex exec --help` or the docs only for uncovered behavior or after a flag or output failure: <https://learn.chatgpt.com/docs/non-interactive-mode> and the [`codex exec` command reference](https://learn.chatgpt.com/docs/developer-commands?surface=cli#cli-codex-exec). For resume flags not listed here, consult `codex exec resume --help`.
