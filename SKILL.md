---
name: spawn-agent
description: Spawn a headless coding-agent CLI run (e.g. `claude -p`) as a subagent for a scoped subtask, from an agent session or a script. Use when the user asks to delegate work to another agent instance, run an agent CLI non-interactively or headlessly, script or automate a headless agent run, add an agent to a build or CI step, or fan work out across isolated sessions; do not use when the host's own subagent, task, or agent tool already fits, or for interactive agent sessions.
---
...
| Harness | CLI | Reference | Status |
| ------- | --- | --------- | ------ |
| Claude Code | `claude` | `references/claude.md` | verified against live runs |

If the user asks for a harness with no reference file here (for example Cursor or Codex), say this skill does not support it yet and offer a supported one. Do not improvise flags for an unsupported CLI from memory; headless flags differ across tools and releases.
